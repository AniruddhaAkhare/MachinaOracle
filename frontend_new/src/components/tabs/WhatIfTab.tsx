...
export default function WhatIfTab({sessionId,machineId}:P){
  ...
  return(
    <div style={{padding:"30px 26px",display:"flex",flexDirection:"column",gap:22}}>
      ...
      <AgentPanel key={`wi_${runKey}_${sel}`} agentType="whatif" sessionId={sessionId} machineId={machineId} agentName="What-If Simulator" fetchFn={()=>getWhatIf(sessionId,machineId,scenario)} cacheKey={`wi_${sessionId}_${machineId}_${sel}_${runKey}`}>
        {(d)=>(
