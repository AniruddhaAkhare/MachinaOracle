...
export default function RootCauseTab({sessionId,machineId}:P){
  return(
    <AgentPanel agentType="rootcause" sessionId={sessionId} machineId={machineId} agentName="Root Cause Analysis" fetchFn={()=>getRootCause(sessionId,machineId)} cacheKey={`rca_${sessionId}_${machineId}`}>
      {(d)=>(
