...
export default function DigitalTwinTab({sessionId,machineId}:P){
  ...
  return(
    <div style={{padding:"30px 26px",display:"flex",flexDirection:"column",gap:22}}>
      ...
      <AgentPanel key={`dt_${key}_${days}`} agentType="digitaltwin" sessionId={sessionId} machineId={machineId} agentName="Digital Twin" fetchFn={()=>getDigitalTwin(sessionId,machineId,days)} cacheKey={`dt_${sessionId}_${machineId}_${days}_${key}`}>
